...
potentially problematic since the use of these% insert image with mislabeled regions
...
\begin{enumerate}
  ...
  \item ReliefF \cite{kononenko1997}. It's an extension of Relief
        ...
        and increases in the reverse case. % TODO: Find references
...
Regardless, it is difficult to determine the quality of these results without
reference data.
...
All supervised methods take a quantitative approach to the problem. Typically,
these methods minimise either one of two errors. On the one hand, instances
incorrectly tagged as mislabeled represent an E1 error. On the other hand,
mislabeled instances left untagged represent an E2 error. Figure
\ref{fig:types-detection-errors} shows a visual representation of the two types of
error.

\begin{figure}[H]
  \centering
  \includegraphics[width=.6\linewidth]{detection_errors_types.png}
  \caption[Types of detection errors.]{Types of detection errors. Adapted from \cite{Brodley1999}.}
...
\pgfplotstabletypeset[
  ...
  columns={Selection Method,Nbr of features used,Accuracy,F-Score,G-Mean}, columns/Selection Method/.style={column type=c,string type},
  columns/Nbr of features used/.style={column type=c,string type}, columns/Accuracy/.style={column type=c,string type},
  columns/F-Score/.style={column type=c,string type}, columns/G-Mean/.style={column type=c,string type},
  every head row/.style={before row=\toprule, after row=\midrule\endhead}, every last row/.style={after row=\bottomrule \\\caption{\label{tab:feature-selection}Feature selection results
        using the Setubal dataset. The number of features selected are based on optimal
        results of each selection method.}}
]{tables/feature_selection_results.csv}

\pgfplotstabletypeset[
  begin table=\begin{longtable},
    end table=\end{longtable}, col sep=comma, header=true,
  columns={Method,Accuracy,F-score,G-mean}, columns/Method/.style={column type=c,string type},
  columns/Accuracy/.style={column type=c,string type}, columns/F-score/.style={column type=c,string type},
  columns/G-mean/.style={column type=c,string type}, every head row/.style={before row=\toprule, after row=\midrule\endhead},
  every last row/.style={after row=\bottomrule \\\caption{\label{tab:COS_sampled_dimreduct_scores}Feature selection results
        using the COS sampled dataset. The number of features selected are based on
        optimal results of each selection method.}}
]{tables/COS_sampled_dimreduct_scores.csv}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=.6\linewidth]{COS_sampled_n_features.png}
  \caption[Evolution of the different scorers on varying number of features (COS sampled
    dataset).]{
    Evolution of the different scorers on varying number of features using the COS
    sampled dataset and the ReliefF method.}
  \label{fig:COS_sampled_n_features}
\end{figure}

Additional analyses based on the COS sampled dataset are presented in tables
\ref{tab:scores_per_band} and \ref{tab:scores_per_month} and figure
\ref{fig:band_month_rank_heatmap}. Note that negative normalized mean scores do not
necessarily mean a negative contribution to the dataset's class separability,
but rather a below average contribution.

\pgfplotstabletypeset[
  begin table=\begin{longtable},
    end table=\end{longtable}, col sep=comma, header=true,
  columns={Band,Normalized Score},  columns/Band/.style={column type=c,string type},
  columns/Normalized Score/.style={column type=c,string type}, every head row/.style={before row=\toprule, after row=\midrule\endhead},
  every last row/.style={after row=\bottomrule \\\caption
      {\label{tab:scores_per_band} Normalized mean importance per band.}}]{tables/scores_per_band.csv}

\pgfplotstabletypeset[
  begin table=\begin{longtable},
    end table=\end{longtable}, col sep=comma, header=true,
  columns={Month,Normalized Score},  columns/Month/.style={column type=c,string type},
  columns/Normalized Score/.style={column type=c,string type}, every head row/.style={before row=\toprule, after row=\midrule\endhead},
  every last row/.style={after row=\bottomrule \\\caption
      {\label{tab:scores_per_month} Normalized mean importance per month.}}]
{tables/scores_per_month.csv}
...
filtered datasets using each of the filters presented. Note that all methods
are tuned for optimal performance on the given dataset, as opposed to the fixed
parameters commonly used in majority, single and consensus filters.

\pgfplotstabletypeset[
  begin table=\begin{longtable},
    end table=\end{longtable}, col sep=comma, header=true,
  columns={Method,0,5,10,20,30,40}, columns/Method/.style={column type=c,string type},
  columns/0/.style={column type=c,string type}, columns/5/.style={column type=c,string type},
  columns/10/.style={column type=c,string type}, columns/20/.style={column type=c,string type},
  columns/30/.style={column type=c,string type}, columns/40/.style={column type=c,string type},
  every head row/.style={before row=\toprule, after row=\midrule\endhead}, every last row/.style={after row=\bottomrule \\\caption{\label{tab:data-filtering}Data filtering results
        using the LUCAS dataset. Column labels represent the different levels of noise
        introduced in the original dataset.}}
]{tables/filtering_results.csv}

A similar experiment was performed on the COS sampled dataset. This experiments
uses the same methods, although the dataset is used as-is (i.e., no noise was
introduced into the dataset). The results of that experiment are reported in
table \ref{tab:anomally_detection_results}.
...
\pgfplotstabletypeset[
  ...
  columns/Accuracy/.style={column type=c,string type}, columns/F-score/.style={column type=c,string type},
  columns/G-mean/.style={column type=c,string type}, every head row/.style={before row=\toprule, after row=\midrule\endhead},
  every last row/.style={after row=\bottomrule \\\caption{\label{tab:anomally_detection_results}Data filtering results
        using the COS sampled dataset.}}
]{tables/anomally_detection_results.csv}
...
\pgfplotstabletypeset[
  ...
  columns={Score,RF,HSN,MLPAE,ResNet50}, columns/Score/.style={column type=c,string type},
  columns/RF/.style={column type=c,string type}, columns/HSN/.style={column type=c,string type},
  columns/MLPAE/.style={column type=c,string type}, columns/ResNet50/.style={column type=c,string type},
  every head row/.style={before row=\toprule, after row=\midrule\endhead}, every last row/.style={after row=\bottomrule \\\caption[Classification scores.]
      {\label{tab:classification-scores}Classification scores using the
        classifiers fitted on the training area (Coimbra dataset) with predictions made
        on the test area, shown in Figure \ref{fig:classifier-study-area}. Legend: Random
        Forest Classifier (RF), Hybrid Spectral Net (HSN), Autoencoder-based multilayer
        perceptron (MLPAE), Residual Neural Network with 50 layers and 2
        fully-connected layers (ResNet50).}}]{tables/classifier_scores.csv}

\pgfplotstabletypeset[
  begin table=\begin{longtable},
    end table=\end{longtable}, col sep=comma, header=true,
  columns={Score,ABC,GBC,LR,RF}, columns/Score/.style={column type=c,string type},
  columns/ABC/.style={column type=c,string type}, columns/GBC/.style={column type=c,string type},
  columns/LR/.style={column type=c,string type}, columns/RF/.style={column type=c,string type},
  every head row/.style={before row=\toprule, after row=\midrule\endhead}, every last row/.style={after row=\bottomrule \\\caption[Classification scores.]
      {\label{tab:COS_sampled_classifier_scores}Classification scores using the COS sampled dataset with
        5-fold cross validation. Legend: AdaBoost Classifier (ABC), Gradient Boosting
        Classifier (GBC), Logistic Regression (LR), Random Forest Classifier (RF).}}]{tables/COS_sampled_classifier_scores.csv}
...
\begin{figure}[H]
  ...
  \caption[Generated maps per classifier.]{Generated maps per classifier. (a) True color image; (b) Ground truth; (c)
    Random Forest Classifier; (d) Hybrid Spectral Net; (e) Autoencoder-based
    multilayer perceptron; (f) Residual Neural Network.}
...
\pgfplotstabletypeset[
  ...
  columns={Step,Algorithm,Observations}, columns/Step/.style={column type=c,string type},
  columns/Algorithm/.style={column type=c,string type}, columns/Observations/.style={column type={p{9cm}},string type},
  every head row/.style={before row=\toprule, after row=\midrule\endhead}, every last row/.style={after row=\bottomrule \\\caption[Classification scores.]
      {\label{tab:cross_temporal_validation}Algorithms used for cross temporal validation}}]{tables/cross_temporal_validation.csv}
...
% Adds reference to the Bibliography in the ToC% Adding a bibliography if citations are used in the report
