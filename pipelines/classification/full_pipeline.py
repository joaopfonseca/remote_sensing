...
center_pixel = (5000,5000)  # Center for the study area
...
del X_lookup
X_test_coords = get_2Dcoordinates_matrix(X_test_lookup.shape, window_size).reshape(2,(margin*2)**2).T
...
for _, split_indices in skf.split(X_test_coords, np.zeros(X_test_coords.shape[0])):
    ...
    X_patches = get_patches(X_split, X_test_lookup, window_size)
...
def pivot_rgb(df, xy_cols, rgb_cols):
    rgb = []
    for band in rgb_cols:
...
ps_rgb = pivot_rgb(df_selected) # to plot selected pixels
...
reports(df_final['y_true'], df_final['y_pred'], labels)
plot_image([
    total_rgb, train_rgb, test_rgb,
    total_gt, train_gt, test_gt,
    train_rgb, ps_rgb, ps_gt,
    test_rgb, test_gt, pred_labels
    ],
